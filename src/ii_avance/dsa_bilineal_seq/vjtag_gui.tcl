...
#
# Diseño:
#  - Acceso a registros: WRITE/READ por DR de 40 bits: {data[31:0], addr[7:0]}.
#  - Lectura de BRAMs: direcciones dedicadas para mem_in/mem_out (solo lectura).
#  - Dos lecturas DR para datos de BRAM tras cambiar dirección (latencia 1 ciclo).
#  - GUI con:
#      * Conectar / Cerrar
#      * Set Params (IN_W, IN_H, SCALE_Q88)
#      * Start (+ flush TAP mínimo)
#      * Read Status / Read IN_W
#      * Dump mem_in / Dump mem_out (incluye “Auto-fill W'·H'”)
# =============================================================================
...
# --- Estado global ---
set IRW  2                ;# ancho IR (bits)
set DRW  40               ;# ancho DR (bits) = 32 data + 8 addr
set IR_WRITE 1            ;# IR para WRITE_REG
set IR_READ  2            ;# IR para READ_REG

set INST -1               ;# instance_index del Virtual JTAG
set __connected 0         ;# bandera conexión abierta
set __hw ""               ;# nombre hardware seleccionado
set __dev ""              ;# nombre device seleccionado

# --- Direcciones de registros (deben coincidir con RTL) ---
...
# BRAM ENTRADA (solo lectura desde UI)
set ADDR_IN_ADDR     0x20 ;# escribir dirección LSBs de mem_in
set ADDR_IN_DATA     0x21 ;# leer dato [7:0] válido en los 8 bits bajos del data[31:0]

# BRAM SALIDA (solo lectura desde UI)
set ADDR_OUT_ADDR    0x30 ;# escribir dirección LSBs de mem_out
set ADDR_OUT_DATA    0x31 ;# leer dato [7:0]

# =============================================================================
# Helpers (parsing / empaquetado / precondiciones)
# =============================================================================

# parse_uint: acepta "123" o "0x7B" y devuelve entero 32b sin signo
proc parse_uint {s} {
  set s [string trim $s]
...
# dr_parse_hex: parsea el retorno de -value_in_hex (exactamente DRW/4 dígitos hex)
proc dr_parse_hex {s} {
  ...
  if {![string is xdigit -strict $s_trim]} {
    error "Respuesta no es HEX válido: '$s_trim'"
  }
...
# pack_dr: empaqueta {data[31:0], addr[7:0]} en entero de 40 bits
...
# require_inst: verifica conexión e instancia seleccionada
proc require_inst {} {
...
# =============================================================================
# Acceso básico Virtual JTAG (IR/DR)
# =============================================================================

# vjtag_ir: shift IR (usa device_lock para exclusión)
proc vjtag_ir {val} {
  require_inst
...
# vjtag_dr: shift DR con -value_in_hex (siempre hex de ancho DRW)
proc vjtag_dr {val40} {
  require_inst
  set hexWidth [expr {$::DRW / 4}]
...
proc read_reg {addr} {
  vjtag_ir $::IR_READ
  # Fase 1: latchear addr
  set dr1 [pack_dr $addr 0]
  vjtag_dr $dr1
  # Fase 2: CAPTURE-DR/SHIFT-DR devuelve {data, addr}; extraer data[31:0] -> >>8
  set rsp [vjtag_dr 0]
...
# =============================================================================
# Lectura BRAM ENTRADA/SALIDA (dos lecturas tras cambiar addr)
# =============================================================================
...
proc read_in_data {} {
  vjtag_ir $::IR_READ
  # primer ciclo (latencia tras cambiar dirección en HW):
  set dr1 [pack_dr $::ADDR_IN_DATA 0]
  vjtag_dr $dr1
  set _   [vjtag_dr 0]
  # segundo ciclo: dato estable -> extraer byte [7:0] (está en data[7:0] => bits [15:8] del DR de 40)
...
# auto_select_instance: busca el primer instance_index válido del IP vJTAG
...
# compute_out_dims: calcula W' y H' en entero con Q8.8 (floor((W*scale)>>8))
...
# dump_mem_in/out: vuelca bloques consecutivos a archivo .hex (una línea por byte)
...
button .f.btnStart -text "Start" -command {
  ...
  # Flush TAP mínimo (lee STATUS una vez)
  catch {
    vjtag_ir $::IR_READ
...
# --- Inicialización de listas y bindings ---
